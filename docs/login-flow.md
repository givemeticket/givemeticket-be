...
`DELETE /api/v1/users/me` — 액세스 토큰 필요. 성공하면 `204`.

- 내가 연 행사는 모두 삭제되고 그 행사의 참가자 전원이 취소·환불된다
- 내가 낸 신청은 모두 취소되고 결제한 건은 환불된다. 비운 자리는 재고로 돌아간다
- 닉네임·프로필 이미지·소셜 회원번호는 지워지고, 신청 이력은 정산 추적을 위해 남는다
- 같은 소셜 계정으로 다시 로그인하면 이전 이력이 없는 **새 계정**으로 가입된다

> 탈퇴해도 이미 발급된 액세스 토큰은 만료 전까지 유효하다. 프론트에서 저장한 토큰을 반드시 지워야 한다.
