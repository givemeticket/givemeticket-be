...
- **카카오 계정은 연결까지 끊긴다.** 카카오 '연결된 서비스'에서 우리 앱이 사라지고, 다시 로그인하면 동의 화면부터 시작한다
...
> 네이버는 연결 끊기를 하지 않는다. 네이버 API 가 사용자의 액세스 토큰을 요구하는데, 로그인 때
> 신원만 확인하고 토큰을 보관하지 않아 탈퇴 시점에는 부를 수단이 없다. 우리 쪽 데이터만 지운다.

### 실패

| 상태 | code | 프론트 처리 |
| --- | --- | --- |
| 404 | `USER_NOT_FOUND` | 이미 탈퇴한 계정. 토큰을 지우고 로그인 화면으로 |
| 500 | `LOGIN_PROVIDER_MISCONFIGURED` | 어드민 키 설정 문제. 재시도해도 소용없으니 백엔드에 알려주세요 |
| 502 | `LOGIN_PROVIDER_UNLINK_ERROR` | 카카오 연결 해제 실패. **아무것도 지워지지 않았으니 그대로 재시도하면 된다** |

연결 끊기는 데이터를 지우기 **전에** 한다. 여기서 실패하면 행사·신청은 그대로 남아 있어
같은 요청을 다시 보내면 된다. 중간에 끊긴 뒤 재시도해도 이중으로 처리되지 않는다.
