...
> **구현 현황** — 2~5절(상태 모델·홀드·정상 플로우·실패 분기), 7절(멱등성), 10~12절은 구현됐다.
> **6절(미결 정산 배치), 8절(재고 정합성 보정), 9절(만료 sweeper), 13절(payment-mock 멱등화),
> 14절(메트릭)은 아직 미구현이다.** 그래서 지금은 `UNKNOWN`으로 떨어진 신청이 스스로 해소되지
> 않고 재고를 잡은 채 남는다. 다음 작업에서 반드시 이어서 해야 한다.
>
...
POST /api/v1/applications/{id}/cancel
  1. 소유자 확인                          → 아니면 403
  2. status == CONFIRMED 인가              → 아니면 409 (PENDING/UNKNOWN은 취소 불가, 확정 대기 중)
  3. 캠페인이 이미 시작/종료됐는가          → 정책에 따라 409 (초기엔 제한 없음)
  4. [tx] CONFIRMED → CANCELLED (조건부 UPDATE)
  5. 재고 복원 (afterCommit)
  6. requiresPayment면 결제 취소 요청 (실패해도 취소는 유지, 별도 재시도 큐)
```

결제 취소가 실패해도 신청 취소는 되돌리지 않는다. 사용자 입장의 취소는 이미 끝났고,
환불은 뒤에서 재시도하면 되는 문제다. 취소 실패 건은 `payment_cancel_failed` 로그 + 메트릭으로 남긴다.
...
| `APPLICATION_NOT_CANCELABLE` | 409 | 취소 불가 상태 |

## 13. payment-mock에 필요한 변경

현행 mock은 상태가 없어서 멱등도 조회도 안 된다. 세 가지를 추가한다.
