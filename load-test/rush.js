...
export function setup() {
  ...
  console.log(
    `campaign=${campaignId}, stock=${STOCK}, requiresPayment=${REQUIRES_PAYMENT}, openAt=${openAt} (UTC)`
  );
...
export default function (data) {
  ...
  check(res, {
    '5xx 없음(201/202/409만)': (r) => r.status === 201 || r.status === 202 || r.status === 409,
  });
}
...
export function teardown(data) {
  ...
  console.log(`정원 = ${STOCK} → apply_created + apply_unknown 이 ${STOCK} 이하이면 정상`);
  console.log('==================================================');
}
